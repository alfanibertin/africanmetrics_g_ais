  ...
  if (bulletins.length === 0) {
    return (
      <div className="bg-brand-input border border-brand-border rounded-2xl p-6 text-center text-brand-dim font-sans shadow-2xs">
        ...
        <p className="text-xs">No active news bulletins. Click "Update Economic Data" above to generate AI-generated bulletins (DeepSeek AI) using active AI models.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4" id="ai-news-bulletins-list">
      {isLive ? (
        <div className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-800 text-xs rounded-xl p-3 flex items-center gap-2.5 font-semibold shadow-2xs">
          ...
          <span>LIVE BULLETINS — Generated dynamically using active DeepSeek AI.</span>
        </div>
      ) : (
        <div className="bg-amber-500/10 border border-amber-500/30 text-amber-800 text-xs rounded-xl p-3 flex items-center gap-2.5 font-semibold shadow-2xs">
          ...
          <span>SIMULATED BULLETINS — Sourced from static baseline. Configure DEEPSEEK_API_KEY in secrets to enable live updates.</span>
        </div>
      )}

      <div className="flex items-center gap-2 mb-1.5">
        <Sparkles className="w-4 h-4 text-[#c2410c]" />
        <h4 className="text-[11px] font-bold font-mono tracking-wider text-[#c2410c] uppercase">AI-generated bulletins (DeepSeek AI)</h4>
      </div>
